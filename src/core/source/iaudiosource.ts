...
import { MicrophoneDevice } from '../../system/microphone';
  ...
  isSilenceDetectionEnabled(): Promise<boolean> {
    return new Promise((resolve) => {
      if (this._isItemCall) {
        ...
        //wrapget
        iItem
          .wrapGet('prop:AudioGainEnable', this._srcId, this._id, this._updateId.bind(this))
          .then((val) => {
            resolve(val === '1');
          });
      }
  ...
  setSilenceDetectionEnabled(value: boolean): Promise<SourceAudio> {
    return new Promise((resolve, reject) => {
      if (this._isItemCall) {
        ...
        iItem.set('prop:AudioGainEnable', value ? '1' : '0', this._id).then((res) => {
          resolve(this);
        });
      } else {
        //wrapset
  ...
  getSilenceThreshold(): Promise<number> {
    return new Promise((resolve) => {
      if (this._isItemCall) {
        ...
        //wrapget
        iItem
          .wrapGet('prop:AudioGain', this._srcId, this._id, this._updateId.bind(this))
          .then((val) => {
            resolve(Number(val));
          });
      }
  ...
  getSilencePeriod(): Promise<number> {
    return new Promise((resolve) => {
      if (this._isItemCall) {
        ...
        iItem
          .wrapGet('prop:AudioGainLatency', this._srcId, this._id, this._updateId.bind(this))
          .then((val) => {
            resolve(Number(val));
          });
      }
  ...
  getAudioOffset(): Promise<number> {
    return new Promise((resolve) => {
      if (this._isItemCall) {
        ...
        iItem
          .wrapGet('prop:AudioDelay', this._srcId, this._id, this._updateId.bind(this))
          .then((val) => {
            resolve(Number(val));
          });
      }
