        ...
        .then((list) => {
          if (typeof value === 'string') {
            if (_playlist.indexOf(value) === -1) {
              ...
              iItem.set('prop:srcitem', file, this._id).then((fileplaylist) => {
                resolve(this);
              });
            }
          } else if (typeof value === 'number' && value <= _playlist.length) {
            file = _playlist[value] + '*' + value;
            iItem.set('prop:srcitem', file, this._id).then(function (fileplaylist) {
              resolve(this);
            });
          } else {
            reject(
  ...
  setVideoPlaylistSources(fileItems: string[]): Promise<SourceVideoPlaylist> {
    ...
    return new Promise((resolve, reject) => {
      Promise.all(filePromises)
        ...
        .then((fileString) => {
          iItem.set('prop:FilePlaylist', fileString, this._id).then((fileplaylist) => {
            resolve(this);
          });
        })
        .catch((err) => {
