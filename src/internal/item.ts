    ...
    return new Promise((resolve) => {
      ...
      const execCallFunc = (slot) => {
        if (
          ...
          (Environment.isSourcePlugin() && String(slot) === '0' && !hasGlobalSources)
        ) {
          slot = -1;
        }
