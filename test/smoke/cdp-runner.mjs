...
const source = await readFile(
  new URL('../../scripts/xsplit-cdp-runner.mjs', import.meta.url),
  'utf8'
);
const attachedSource = await readFile(
...
assert.match(
  source,
  /localhost:3999\/xsplit-extension\//,
  'runner should select the XSplit extension target'
);
...
assert.match(
  source,
  /screenshot\.png/,
  'runner should store screenshots for successful visual regression runs'
);
assert.match(
  source,
  /collectPageState/,
  'runner should collect page state when the suite cannot bootstrap'
);
assert.match(source, /runner-bootstrap/, 'runner should emit a structured bootstrap failure');
assert.match(
  source,
  /runnerType/,
  'bootstrap diagnostics should include the regression runner type'
);
assert.match(
  source,
  /scripts:\s*Array\.from\(document\.scripts\)/,
  'bootstrap diagnostics should include loaded scripts'
);
...
assert.match(
  source,
  /latest-summary\.json/,
  'runner should write a stable latest artifact summary'
);
assert.match(
  source,
  /artifactDirectory/,
  'latest summary should point at the timestamped artifact directory'
);
assert.match(source, /passed/, 'latest summary should record pass/fail state');
assert.doesNotMatch(
  source,
  /if\s*\(\s*failures\.length[\s\S]{0,300}Page\.captureScreenshot/,
  'screenshot capture should not be limited to failing runs'
);
assert.doesNotMatch(source, /connectOverCDP|playwright/i, 'XSplit runner should be raw CDP first');

assert.match(
  attachedSource,
  /XJS_EXTENSION_FALLBACK_URL_PREFIXES/,
  'attached runner should configure fallback target prefixes'
);
assert.match(
  attachedSource,
  /XJS_EXTENSION_NAVIGATE_URL/,
  'attached runner should configure extension navigation URL'
);
assert.match(
  attachedSource,
  /localhost:3999\/xsplit-extension\/index\.html/,
