...
assert.match(
  ...
  'docs app should keep the same custom element tag'
);

assert.match(
  navbar,
  /customElements\.define\(['"]xsplit-navbar['"]/,
  'navbar module should define xsplit-navbar'
);
assert.match(
  navbar,
  /class\s+XSplitNavbar\s+extends\s+HTMLElement/,
  'navbar should be a native custom element'
);
