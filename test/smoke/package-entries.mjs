...
for (const file of expectedFiles) {
  assert.equal(
    existsSync(join(dist.pathname, file)),
    true,
    `expected dist/${file} to exist`
  );
}
...
assert.match(browserBundle, /chrome103|CEF 103|xjs-framework/i, 'browser bundle should retain an identifiable banner');
