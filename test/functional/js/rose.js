...
var Rose = (() => {
  ...
  Rose.prototype.initEvents = function () {
    ...
    this._tabGroup.addEventListener('click', (event) => {
      var element = event.target;
      if (element.tagName === 'LI') {
        var target = element.getAttribute('tab-id');
        var curActive = this._tabGroup.querySelector('.active');
        if (curActive && curActive.getAttribute('tab-id') !== target) {
          this._tabGroup.querySelector('.active').classList.remove('active');
          element.classList.add('active');
        }
        element.classList.add('active');
        this.setContent(this._contents['find']((elem) => elem['id'] === target));
      }
    });
  };
  /**
  ...
  Rose.prototype.generateID = function () {
    ...
    if (this._contents['find']((elem) => elem['id'] === _id) === undefined) {
      return _id;
    } else {
