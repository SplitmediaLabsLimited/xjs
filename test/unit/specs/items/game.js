...
describe('GameItem', () => {
    ...
    switch (funcName) {
      ...
      case 'GameCapSurfSharing':
        if (Object.hasOwn(local, 'GameCapSurfSharing')) {
          ...
          setTimeout(() => {
            var irand = rand;
            window.OnAsyncCallback(irand, itemSelected.getAttribute('GameCapSurfSharing'));
          }, 10);
        }
        break;
      ...
      case 'GameCapShowMouse':
        if (Object.hasOwn(local, 'GameCapShowMouse')) {
          ...
          var irand = rand;
          setTimeout(() => {
            window.OnAsyncCallback(irand, itemSelected.getAttribute('GameCapShowMouse'));
          }, 10);
        }
        break;
  ...
  describe('gameItem-specific methods checking', () => {
    beforeEach(function (done) {
      if (enumerated.length > 0) {
        currentGameItem = enumerated[0];
        done();
    ...
    it('should be able to check whether special optimization is enabled or not', (done) => {
      exec((next) => {
    ...
    it('should be able to enable or disable special optimization', (done) => {
      var testBoolean = randomBoolean();
      exec((next) => {
        currentGameItem
          .setSpecialOptimizationEnabled(testBoolean)
          .then(() => {
            if (!isXSplit) {
    ...
    it('should be able to check whether mouse is shown or not in game capture', (done) => {
      exec((next) => {
    ...
    it('should be able to show or hide mouse in game capture', (done) => {
      var testBoolean = randomBoolean();
      exec((next) => {
        currentGameItem
          .setShowMouseEnabled(testBoolean)
          .then(() => {
            if (!isXSplit) {
    ...
    it('should be able to get offline image', (done) => {
      exec((next) => {
        var promise = currentGameItem.getOfflineImage();
        expect(promise).toBeInstanceOf(Promise);
        promise.then((offlineImage) => {
    ...
    it('should be able to set offline image', (done) => {
      var firstPath = 'C:\\someFolder\\someFile.jpg';
      var secondPath = 'C:\\anotherFolder\\anotherFile.jpg';

      exec((next) => {
        currentGameItem.setOfflineImage(firstPath).then((item) => {
          item
            .getOfflineImage()
            .then((offlineImage) => {
