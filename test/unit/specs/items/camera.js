...
describe('CameraItem', () => {
      ...
      switch (funcName) {
        case 'dshowenum:vsrc':
          xCallback(
            asyncId,
            encodeURIComponent(
              '<list><dev disp="clsid:{39F50F4C-99E1-464A-B6F9-D605B4FB5918}" name="Elgato Game Capture HD"/><dev disp="@device:pnp:\\\\?\\usb#vid_046d&amp;pid_082c&amp;mi_02#6&amp;37c59c5d&amp;0&amp;0002#{65e8773d-8f56-11d0-a3b9-00a0c9223196}\\global" name="HD Webcam C615"/></list>'
            )
          );
          break;
  ...
  describe('cameraItem-specific methods checking, ', () => {
    afterEach(() => {
      navigator.__defineGetter__('appVersion', () => appVersion);
    });
      ...
      exec((next) => {
        ...
        promise
          .then((isPaused) => {
            ...
            pauseValue = !isPaused;
            shouldFail = true;
            return this.cameraItem.setStreamPaused(pauseValue);
          })
          .then(() => {
          ...
          .then(() => this.cameraItem.isStreamPaused())
          .then((val) => {
    ...
    it('should be able to get and set audio input', function (done) {
      exec((next) => {
        var promise = this.cameraItem.getAudioInput();
        var micDevice;
        promise
          .then((audioInput) => {
          ...
          .then((micDevices) => {
            ...
            return this.cameraItem.setAudioInput(micDevice);
          })
          .then(() => this.cameraItem.getAudioInput())
          .then((newDevice) => {
            expect(newDevice).toEqual(micDevice);
            shouldFail = true;
            return this.cameraItem.getAudioInput();
          })
          .then(() => {
    ...
    it('should be able to check if hardware encoder or not', function (done) {
      exec((next) => {
        shouldFail = false;
        shouldFail2 = false;
        var promise = this.cameraItem.isHardwareEncoder();

        promise
          .then((isHardwareEncoder) => {
            expect(isHardwareEncoder).toBeTypeOf('boolean');
            expect(isHardwareEncoder).toBe(true);
            shouldFail = true;
            return this.cameraItem.isHardwareEncoder();
          })
          .then((isHardwareEncoder2) => {
    ...
    it('should be able to check if active or not', function (done) {
      exec((next) => {
        var promise = this.cameraItem.isActive();

        promise.then((isActive) => {
