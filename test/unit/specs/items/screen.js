...
describe('ScreenItem', () => {
  ...
  describe('ScreenItem-specific methods checking', () => {
    beforeAll((done) => {
      if (enumerated.length > 0) {
        currentScreenItem = enumerated[0];
        done();
    ...
    it('should be able to get the capture area', (done) => {
      exec((next) => {
        var promise = currentScreenItem.getCaptureArea();
        expect(promise).toBeInstanceOf(Promise);
        promise.then((captureArea) => {
    ...
    it('should be able to set the capture area', (done) => {
      exec((next) => {
        var rect = XJS.Rectangle.fromCoordinates(0, 0, 1920, 1080);
        local.item = undefined;
        currentScreenItem.setCaptureArea(rect).then(() => {
    ...
    it('should be able to get the client area value', (done) => {
      exec((next) => {
        currentScreenItem.isClientArea().then((val) => {
    ...
    it('should be able to set the client area value', (done) => {
      exec((next) => {
        local.item = undefined;
        currentScreenItem.setClientArea(true).then(() => {
          expect(local.item).toBeTypeOf('string');
          next();
        });
      }).then(done);
    ...
    it('should be able to get the stick to title value', (done) => {
      exec((next) => {
        currentScreenItem.isStickToTitle().then((val) => {
          expect(val).toBeTypeOf('boolean');
          next();
        });
      }).then(done);
    ...
    it('should be able to set the stick to title value', (done) => {
      exec((next) => {
        local.item = undefined;
        currentScreenItem.setStickToTitle(true).then(() => {
