...
describe('Item ===', () => {
  ...
  var getLocal = (prop) => {
    ...
    switch (prop) {
      ...
      case 'prop:srcid':
        setTimeout(() => {
          window.OnAsyncCallback(asyncId, '{CB4EB352-D86F-4478-8BFD-55FF53216697}');
        }, 10);
        break;
  ...
  it('should be able to set and get the name', (done) => {
    var word = randomWord(5);
    exec((next) => {
      Item.setName(word);
      Item.getName().then((val) => {
  ...
  it('should be able to set and get the custom name', (done) => {
    var word = randomWord(5);
    exec((next) => {
      Item.setCustomName(word);
      Item.getCustomName().then((val) => {
        expect(val).toEqual(word);
        next();
      });
    }).then(done);
  ...
  it('should be able to set and get the value', (done) => {
    var word = randomWord(5);
    exec((next) => {
      Item.setValue(word);
  ...
  it('should be able to set and get keep loaded property', (done) => {
    exec((next) => {
      Item.setKeepLoaded(!local.keeploaded);
      Item.getKeepLoaded().then((val) => {
  ...
  it('should be able to get the id', (done) => {
    exec((next) => {
      Item.getId().then((val) => {
  ...
  it('should be able to get its source', (done) => {
    exec((next) => {
      Item.getSource().then((source) => {
  ...
  it('should be able to get the scene id', (done) => {
    exec((next) => {
      Item.getSceneId().then((val) => {
  ...
  it('should have toXML method', () => {
    expect(Item).hasMethods('toXML');
  });

  it('should have getFPS method', (done) => {
    expect(Item).hasMethods('getFPS');
    exec((next) => {
      Item.getFPS().then((val) => {
        expect(val).toBeTypeOf('number');
        expect(val).not.toBeNaN();
        next();
      });
  ...
  it('shoud have static getItemList method', () => {
  ...
  describe('interface method checking', () => {
    ...
    it('should implement the iSource interface', () => {
      expect(Item).hasMethods(
        [
          'setName',
          'getName',
