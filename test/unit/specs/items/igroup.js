...
describe('Group interface', () => {
  ...
  beforeEach((done) => {
    ...
    spyOn(window.external, 'AppGetPropertyAsync').and.callFake((funcName) => {
      ...
      } else if (funcName.startsWith('scenecanremovefromgroup')) {
        if (canRemoveShouldFail) {
          xCallback(asyncId, '0');
        } else {
          xCallback(asyncId, '1');
        }
      } else if (funcName.startsWith('sceneconfig:')) {
        xCallback(asyncId, encodeURIComponent(mockScene2Config));
      } else {
        switch (funcName) {
          case 'sceneconfig':
    ...
    if (enumeratedItems.length !== 0) {
      ...
      Scene.getActiveScene()
        .then((newScene) => newScene.getTopLevelItems())
        .then((items) => {
          var itemArray = items;
          ...
          if (itemArrayLength > 0) {
            for (var i = 0; i < itemArrayLength; i++) {
              if (itemArray[i] instanceof XJS.GroupItem) {
                ...
                nonChild = itemArray[i];
              }
            }
          }
          firstItem = enumeratedItems[0];

          firstItem.getItems().then((items) => {
            childItems = items;
            done();
          });
        });
  ...
  it('contains all the necessary group methods', () => {
    var methods = ['getItems', 'addItems', 'removeItems', 'unGroup'].join(',');

    expect(firstItem).hasMethods(methods);
  });

  describe('should be able to get its own child items', () => {
  ...
  describe('should be able to add items into it', () => {
    it('through a promise', (done) => {
      var promise = firstItem.addItems();
      expect(promise).toBeInstanceOf(Promise);
      done();
    });

    it('as an Item or array of Items', (done) => {
      var childArray = [];
      childArray.push(nonChild);
      firstItem
        .addItems(nonChild)
        .then((item) => {
          expect(item).toBeInstanceOf(XJS.GroupItem);
          return firstItem.addItems(childArray);
        })
        .then((item) => {
          expect(item).toBeInstanceOf(XJS.GroupItem);
          done();
    ...
    it('as an string or array of Strings', (done) => {
      var nonChildId = nonChild._id;
      var childArray = [];
      childArray.push(nonChildId);
      firstItem
        .addItems(nonChildId)
        .then((item) => {
          expect(item).toBeInstanceOf(XJS.GroupItem);
    ...
    it('which should reject on failure', (done) => {
      getSceneShouldFail = true;
      firstItem
        .addItems(nonChild)
        .then((items) => {
          done.fail('addItems should reject when Item is no longer available');
        })
        .catch((err) => {
          expect(err).toEqual(jasmine.any(Error));
          getSceneShouldFail = false;
          canAddShouldFail = true;
          return firstItem.addItems(nonChild);
        })
        .then((items) => {
          done.fail(
            'addItems should reject when at least one of the items cannot be added to the group'
          );
        })
        .catch((err) => {
          expect(err).toEqual(jasmine.any(Error));
          canAddShouldFail = false;
          return firstItem.addItems(nonChild);
        })
        .then((item) => {
          expect(item).toBeInstanceOf(XJS.GroupItem);
  ...
  describe('should be able to remove its child items', () => {
    it('through a promise', (done) => {
      var promise = firstItem.removeItems(childItems);
      expect(promise).toBeInstanceOf(Promise);
      done();
    });
    ...
    it('as an Item or array of Items', (done) => {
      var childArray = [];
      firstItem
        .removeItems(childItems[0])
        .then((item) => {
          expect(item).toBeInstanceOf(XJS.GroupItem);
          return firstItem.removeItems(childItems);
        })
        .then((item) => {
          expect(item).toBeInstanceOf(XJS.GroupItem);
          done();
    ...
    it('as an string or array of Strings', (done) => {
      ...
      firstItem
        ...
        .then((item) => {
          expect(item).toBeInstanceOf(XJS.GroupItem);
          return firstItem.removeItems(childArray);
        })
        .then((item) => {
          expect(item).toBeInstanceOf(XJS.GroupItem);
          done();
    ...
    it('which should reject on failure', (done) => {
      getSceneShouldFail = true;
      firstItem
        .removeItems(childItems)
        .then((items) => {
          done.fail('removeItems should reject when Item is no longer available');
        })
        .catch((err) => {
          expect(err).toEqual(jasmine.any(Error));
          getSceneShouldFail = false;
          canRemoveShouldFail = true;
          return firstItem.removeItems(childItems);
        })
        .then((items) => {
          done.fail(
            'removeItems should reject when at least one of the items cannot be removed from the group'
          );
        })
        .catch((err) => {
          expect(err).toEqual(jasmine.any(Error));
          canRemoveShouldFail = false;
          return firstItem.removeItems(childItems);
        })
        .then((item) => {
          expect(item).toBeInstanceOf(XJS.GroupItem);
  ...
  describe('should be able to ungroup', () => {
    it('as a promise', (done) => {
