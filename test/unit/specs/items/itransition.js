...
describe('Transition interface', function() {
  ...
  describe('should be able to get and set visibility', function() {
    it ('through a promise', function(done) {
      var promise = firstItem.isVisible();
    ...
    it ('as a boolean', function(done) {
      ...
      firstItem.setVisible(firstBoolean)
      .then(function() {
        return secondItem.setVisible(secondBoolean);
      }).then(function() {
        return firstItem.isVisible();
      }).then(function(visible1) {
        expect(visible1).toBeTypeOf('boolean');
        expect(visible1).toEqual(firstBoolean);
        return secondItem.isVisible();
      }).then(function(visible2) {
        expect(visible2).toBeTypeOf('boolean');
        expect(visible2).toEqual(secondBoolean);
        return firstItem.setVisible(!firstBoolean);
      }).then(function() {
        return firstItem.isVisible();
      }).then(function(visible3) {
        expect(visible3).toEqual(!firstBoolean);
        done();
      })
    });
  });

  describe('should be able to get and set source transitions', function() {
    it ('as a promise', function(done) {
    	var promise = firstItem.getTransition();
    	expect(promise).toBeInstanceOf(Promise);
    	done();
    });

    it ('as a Transition object', function(done) {
	    var transitionKeys = Object.keys(Transition._transitionMap);
	    var randomTransition = transitionKeys[Math.floor(Math.random()*transitionKeys.length)];
	    var randomTransitionObj = Transition[randomTransition];

	    var randomTransition2 = transitionKeys[Math.floor(Math.random()*transitionKeys.length)];
	    var randomTransitionObj2 = Transition[randomTransition2];

      firstItem.setTransition(randomTransitionObj)
      .then(function() {
        return secondItem.setTransition(randomTransitionObj2);
      }).then(function() {
        return firstItem.getTransition();
      }).then(function(transition1) {
        expect(transition1).toBeInstanceOf(Transition);
        expect(transition1.toString()).toEqual(randomTransitionObj.toString());
        return secondItem.getTransition();
      }).then(function(transition2) {
        expect(transition2).toBeInstanceOf(Transition);
        expect(transition2.toString()).toEqual(randomTransitionObj2.toString());
        done();
      })
    });
    ...
    it ('which rejects for invalid parameters', function(done) {
      ...
      firstItem.setTransition(randomString)
      .then(function() {
        done.fail('Invalid type was accepted (string)');
      }).catch(function(err) {
      	expect(err).toEqual(jasmine.any(Error));
        return firstItem.setTransition(randomNumber)
      }).then(function() {
  ...
  describe('should be able to get and set transition time', function() {
    it ('through a promise', function(done) {
      var promise = firstItem.getTransitionTime();
      expect(promise).toBeInstanceOf(Promise);
      done();
    });

    it ('as a number', function(done) {
      var firstRand = randomInt(1,360);
      var secondRand = randomInt(1, 60000);

      firstItem.setTransitionTime(firstRand)
      .then(function() {
        return secondItem.setTransitionTime(secondRand);
      }).then(function() {
        return firstItem.getTransitionTime();
      }).then(function(time1) {
        expect(time1).toBeTypeOf('number');
        expect(time1).toEqual(firstRand);
        return secondItem.getTransitionTime();
      }).then(function(time2) {
        expect(time2).toBeTypeOf('number');
        expect(time2).toEqual(secondRand);
        done();
      })
    });
    ...
    it ('which rejects for invalid parameters', function(done) {
    	var randomString = randomWord(5);
    	var randomNumber = randomInt(60001, 100000);

      firstItem.setTransitionTime(randomString)
      .then(function() {
        done.fail('Invalid type was accepted (string)');
      }).catch(function(err) {
      	expect(err).toEqual(jasmine.any(Error));
        return firstItem.setTransitionTime(randomNumber)
      }).then(function() {
      	done.fail('Invalid value was accepted');
      }).catch(function(err) {
        expect(err).toEqual(jasmine.any(Error));
