...
describe('Camera', () => {
  ...
  describe('should list camera devices', () => {
    var promise;
    ...
    it('with correct properties', (done) => {
      promise.then((devices) => {
        expect(devices).eachHasMethods(['getName', 'getId'].join(','));
        done();
      });
    });

    it('excludes XSplit camera devices, Intel realsense and Tridef Smart Cam', (done) => {
      promise.then((devices) => {
        for (var i = devices.length - 1; i >= 0; i--) {
          ...
          expect(
            devices[i]
              .getName()
              .toLowerCase()
              .indexOf('Intel(R) RealSense(TM) 3D Camera Virtual Driver'.toLowerCase())
          ).toBe(-1);
          expect(
            devices[i]
              .getName()
          ...
          expect(
            devices[i]
              .getId()
              .toLowerCase()
              .indexOf('@DEVICE:PNP:\\\\?\\USB#VID_8086&PID_0AA5&MI_02#'.toLowerCase())
          ).toBe(-1);
          expect(
            devices[i]
              .getId()
              .toLowerCase()
              .indexOf('@DEVICE:PNP:\\\\?\\USB#VID_8086&PID_0A66&MI_02#'.toLowerCase())
          ).toBe(-1);
          expect(devices[i].getId().toLowerCase()).not.toEqual(
            (
  ...
  describe('system camera devices', () => {
    var promise;
