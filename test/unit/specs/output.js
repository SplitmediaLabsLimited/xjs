...
describe('Output ===', () => {
  ...
  var mockOutputList =
    '<channels><channel name="Local Recording" /><channel name="Local Streaming" displayName="Local Streaming"/><channel name="Twitch - TwitchChannel" displayName="Twitch - TwitchChannel" /></channels>';
  ...
  var mix = new window.Mixin([
    ...
    () => {
      navigator.__defineGetter__('appVersion', () => 'XSplit Broadcaster 3.1.1707.3101 ');
    },
  ]);
  ...
  beforeEach(() => {
    ...
    spyOn(external, 'CallHostFunc').and.callFake(function () {
      var funcName = arguments[0];
      global_asyncId++;
      if (funcName.startsWith('getBroadcastChannelList')) {
      ...
      if (funcName === 'itemlist') {
        ...
        setTimeout(() => {
          window.OnAsyncCallback(
            asyncId,
            '{76D1C7FC-4CF2-4D7C-88F7-AABC6EE40705},{B438A6F2-2035-4F58-B011-131BBA706F31}'
          );
        }, 10);
        return asyncId;
      }
  ...
  describe('should fetch all available outputs', () => {
    ...
    it('that returns an array of Outputs', (done) => {
      exec((next) => {
        Output.getOutputList().then((outputs) => {
    ...
    it('should reject when called in source properties', (done) => {
      navigator.__defineGetter__('appVersion', () => appVersion);
      env.set(environments[0]);
      navigator.__defineGetter__('appVersion', () => 'XSplit Broadcaster 2.8.1603.0401 ');
      Output.getOutputList().then(
        (source) => {
  ...
  describe('should have different broadcast-related methods', () => {
    ...
    it('such as getting output name', (done) => {
      localRecording
        .getName()
        .then((name) => {
    ...
    // if this fails, please first check if the expected value corresponds to the supplied channel list
    // Local Recording should always be first
    it('such as getting output display name', (done) => {
      localRecording
        .getDisplayName()
        .then((name) => {
          expect(name).toEqual('Local Recording');
    ...
    it('starting and stopping broadcasts', (done) => {
      exec2((next) => {
        otherOutput
          .startBroadcast()
          .then(() => {
    ...
    it('pausing and unpausing of local recording', (done) => {
      otherOutput
        .pauseLocalRecording()
        .then(
          (source) => {
          ...
          () => otherOutput.unpauseLocalRecording()
        )
        .then(
          (source) => {
            done.fail('unpause should reject if called from non-Local Recording');
          },
          () => localRecording.pauseLocalRecording()
        )
        .then(() => {
    ...
    it('pausing and unpausing of local recording should just work', (done) => {
