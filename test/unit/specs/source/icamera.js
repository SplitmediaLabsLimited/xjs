...
describe('CameraSource interface', () => {
  ...
  it('contains all the necessary camera methods', () => {
    var methods = [
      'getDeviceId',
      'getAudioOffset',
      'setAudioOffset',
  ...
  describe('should be able to get device ID', () => {
  ...
  describe('should be able to get camera resolution', () => {
    it('through a promise', (done) => {
    ...
    it('as a rectangle', (done) => {
      firstSource.getResolution().then((resolution) => {
  ...
  describe('should be able to get and set audio offset', () => {
    it('through a promise', (done) => {
    ...
    it('as a number', (done) => {
      ...
      firstSource
        .setAudioOffset(firstRand)
        .then(() => secondSource.setAudioOffset(secondRand))
        .then(() => firstSource.getAudioOffset())
        .then((audioOffset1) => {
  ...
  describe('should be able to get and set audio input', () => {
    it('through a promise', (done) => {
    ...
    it('as a microphone device', (done) => {
      secondSource.getAudioInput().then((audioInput) => {
        expect(audioInput).toBeInstanceOf(XJS.MicrophoneDevice);
        done();
      });
    });

    it('which rejects when no audio input tied', (done) => {
      firstSource
        .getAudioInput()
        .then((audioInput) => {
        ...
        .then((mics) => firstSource.setAudioInput(mics[0]))
        .then(() => firstSource.getAudioInput())
        .then((newAudioInput) => {
    ...
    it('which rejects when tied audio input is not present', (done) => {
  ...
  describe('should be able to get and set stream pause state', () => {
    ...
    it('as a boolean', (done) => {
      firstSource
        .setStreamPaused(randomBool)
        .then(() => firstSource.isStreamPaused())
        .then((isPaused) => {
  ...
  describe('should be able to get hardware encoding capability and active state', () => {
    it('through a promise', (done) => {
    ...
    it('which rejects if device hardware encoding capability cannot be determined due to absence', (done) => {
      firstSource
        .isHardwareEncoder()
        .then((isHwEncoder) => {
        ...
        .then(() => {
            ...
            'It should reject if hw encoding capability cannot be determined due to absence'
          );
        })
        .catch((err) => {
          expect(err).toEqual(jasmine.any(Error));
  ...
  describe('should be able to get and set delay', () => {
    it('through a promise', (done) => {
    ...
    it('as a number', (done) => {
      ...
      firstSource
        .setDelay(firstRand)
        .then(() => secondSource.setDelay(secondRand))
        .then(() => firstSource.getDelay())
        .then((delay1) => {
    ...
    it('which rejects when setting to invalid sources', (done) => {
      shouldFailHWCheck = true;
      isCamActive = true;
