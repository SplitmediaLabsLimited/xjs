...
describe('Scene Source interface', () => {
  ...
  it('contains all the necessary scene source methods', () => {
    var methods = ['getScene', 'setScene'].join(',');

    expect(enumeratedSource[0]).hasMethods(methods);
  });

  describe('should be able to get and set scene it references', () => {
    ...
    it('as a Scene object', (done) => {
      ...
      firstSource
        ...
        .then((sceneUid) => secondSource.setScene(XJS.Scene.liveScene()))
        .then(() => firstSource.getScene())
        .then((scene2) => {
